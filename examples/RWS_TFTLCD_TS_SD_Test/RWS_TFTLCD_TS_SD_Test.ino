// Example program for the RWS_TFTLCD_TS library with write access to the SD card by Rick Sellens CC license.
// see README.txt
//
...
#define SD_CS 5      // Card Select for shield
...
boolean    sdValid = 0, sdFileValid = 0;
File       logfile;
...
void setup(void) {
  ...
  if (!SD.begin(SD_CS)){ 
    ...
    // create new file
    char logfilename[] = "TSTL0000.TXT";
    for (uint8_t i = 0; i < 100; i++) {
      logfilename[6] = i/10 + '0';
      logfilename[7] = i%10 + '0';
      if (! SD.exists(logfilename)) {
        // only open new file if both don't exist
...
void findEdges(uint16_t c1){
  ...
  while(millis() - startTime < 30000){
    ...
    if(p.z != 0){
      ...
      if(sdFileValid){
        Serial.println("About to print to SD");
        //********************* Switch to SD mode, print to the file, flush, then switch back ***************
        SPCR = spi_save_sd;
        logfile.print("   X= "); logfile.print(p.x); logfile.print(" Y= "); logfile.print(p.y); 
        logfile.print(" Z= "); logfile.println(p.z); logfile.flush();
        SPCR = spi_save_tft;
        //***************************************************************************************************
      }
    vals[0]= p.x/12;
